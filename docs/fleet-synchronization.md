...
selected-repository visibility with exactly that set. The audit performs only
GitHub API reads, never requests secret values, and fails closed on missing,
extra, incomplete, malformed, or unavailable metadata.
